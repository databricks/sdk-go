// Code generated by Databricks SDK Generator. DO NOT EDIT.

package queryhistory

type channelInfoWire struct {
	Name         ChannelName `json:"name,omitempty"`
	DbsqlVersion *string     `json:"dbsql_version,omitempty"`
}

func channelInfoToWire(v *ChannelInfo) *channelInfoWire {
	if v == nil {
		return nil
	}
	return &channelInfoWire{
		Name:         v.Name,
		DbsqlVersion: v.DbsqlVersion,
	}
}

func channelInfoFromWire(w *channelInfoWire) *ChannelInfo {
	if w == nil {
		return nil
	}
	return &ChannelInfo{
		Name:         w.Name,
		DbsqlVersion: w.DbsqlVersion,
	}
}

type externalQuerySourceWire struct {
	DashboardId       *string                          `json:"dashboard_id,omitempty"`
	LegacyDashboardId *string                          `json:"legacy_dashboard_id,omitempty"`
	AlertId           *string                          `json:"alert_id,omitempty"`
	NotebookId        *string                          `json:"notebook_id,omitempty"`
	SqlQueryId        *string                          `json:"sql_query_id,omitempty"`
	JobInfo           *externalQuerySource_JobInfoWire `json:"job_info,omitempty"`
	GenieSpaceId      *string                          `json:"genie_space_id,omitempty"`
}

func externalQuerySourceToWire(v *ExternalQuerySource) *externalQuerySourceWire {
	if v == nil {
		return nil
	}
	return &externalQuerySourceWire{
		DashboardId:       v.DashboardId,
		LegacyDashboardId: v.LegacyDashboardId,
		AlertId:           v.AlertId,
		NotebookId:        v.NotebookId,
		SqlQueryId:        v.SqlQueryId,
		JobInfo:           externalQuerySource_JobInfoToWire(v.JobInfo),
		GenieSpaceId:      v.GenieSpaceId,
	}
}

func externalQuerySourceFromWire(w *externalQuerySourceWire) *ExternalQuerySource {
	if w == nil {
		return nil
	}
	return &ExternalQuerySource{
		DashboardId:       w.DashboardId,
		LegacyDashboardId: w.LegacyDashboardId,
		AlertId:           w.AlertId,
		NotebookId:        w.NotebookId,
		SqlQueryId:        w.SqlQueryId,
		JobInfo:           externalQuerySource_JobInfoFromWire(w.JobInfo),
		GenieSpaceId:      w.GenieSpaceId,
	}
}

type externalQuerySource_JobInfoWire struct {
	JobId        *string `json:"job_id,omitempty"`
	JobRunId     *string `json:"job_run_id,omitempty"`
	JobTaskRunId *string `json:"job_task_run_id,omitempty"`
}

func externalQuerySource_JobInfoToWire(v *ExternalQuerySource_JobInfo) *externalQuerySource_JobInfoWire {
	if v == nil {
		return nil
	}
	return &externalQuerySource_JobInfoWire{
		JobId:        v.JobId,
		JobRunId:     v.JobRunId,
		JobTaskRunId: v.JobTaskRunId,
	}
}

func externalQuerySource_JobInfoFromWire(w *externalQuerySource_JobInfoWire) *ExternalQuerySource_JobInfo {
	if w == nil {
		return nil
	}
	return &ExternalQuerySource_JobInfo{
		JobId:        w.JobId,
		JobRunId:     w.JobRunId,
		JobTaskRunId: w.JobTaskRunId,
	}
}

type listQueriesRequestWire struct {
	FilterBy       *queryFilterWire `json:"filter_by,omitempty"`
	MaxResults     *int             `json:"max_results,omitempty"`
	PageToken      *string          `json:"page_token,omitempty"`
	IncludeMetrics *bool            `json:"include_metrics,omitempty"`
}

func listQueriesRequestToWire(v *ListQueriesRequest) *listQueriesRequestWire {
	if v == nil {
		return nil
	}
	return &listQueriesRequestWire{
		FilterBy:       queryFilterToWire(v.FilterBy),
		MaxResults:     v.MaxResults,
		PageToken:      v.PageToken,
		IncludeMetrics: v.IncludeMetrics,
	}
}

func listQueriesRequestFromWire(w *listQueriesRequestWire) *ListQueriesRequest {
	if w == nil {
		return nil
	}
	return &ListQueriesRequest{
		FilterBy:       queryFilterFromWire(w.FilterBy),
		MaxResults:     w.MaxResults,
		PageToken:      w.PageToken,
		IncludeMetrics: w.IncludeMetrics,
	}
}

type listQueriesResponseWire struct {
	NextPageToken *string         `json:"next_page_token,omitempty"`
	HasNextPage   *bool           `json:"has_next_page,omitempty"`
	Res           []queryInfoWire `json:"res,omitempty"`
}

func listQueriesResponseToWire(v *ListQueriesResponse) *listQueriesResponseWire {
	if v == nil {
		return nil
	}
	return &listQueriesResponseWire{
		NextPageToken: v.NextPageToken,
		HasNextPage:   v.HasNextPage,
		Res:           convertSlice(v.Res, queryInfoToWire),
	}
}

func listQueriesResponseFromWire(w *listQueriesResponseWire) *ListQueriesResponse {
	if w == nil {
		return nil
	}
	return &ListQueriesResponse{
		NextPageToken: w.NextPageToken,
		HasNextPage:   w.HasNextPage,
		Res:           convertSlice(w.Res, queryInfoFromWire),
	}
}

type queryFilterWire struct {
	QueryStartTimeRange *timeRangeWire `json:"query_start_time_range,omitempty"`
	UserIds             []int64        `json:"user_ids,omitempty"`
	Statuses            []QueryStatus  `json:"statuses,omitempty"`
	WarehouseIds        []string       `json:"warehouse_ids,omitempty"`
	StatementIds        []string       `json:"statement_ids,omitempty"`
}

func queryFilterToWire(v *QueryFilter) *queryFilterWire {
	if v == nil {
		return nil
	}
	return &queryFilterWire{
		QueryStartTimeRange: timeRangeToWire(v.QueryStartTimeRange),
		UserIds:             v.UserIds,
		Statuses:            v.Statuses,
		WarehouseIds:        v.WarehouseIds,
		StatementIds:        v.StatementIds,
	}
}

func queryFilterFromWire(w *queryFilterWire) *QueryFilter {
	if w == nil {
		return nil
	}
	return &QueryFilter{
		QueryStartTimeRange: timeRangeFromWire(w.QueryStartTimeRange),
		UserIds:             w.UserIds,
		Statuses:            w.Statuses,
		WarehouseIds:        w.WarehouseIds,
		StatementIds:        w.StatementIds,
	}
}

type queryInfoWire struct {
	QueryId            *string                  `json:"query_id,omitempty"`
	Status             QueryStatus              `json:"status,omitempty"`
	QueryText          *string                  `json:"query_text,omitempty"`
	QueryStartTimeMs   *int64                   `json:"query_start_time_ms,omitempty"`
	ExecutionEndTimeMs *int64                   `json:"execution_end_time_ms,omitempty"`
	QueryEndTimeMs     *int64                   `json:"query_end_time_ms,omitempty"`
	UserId             *int64                   `json:"user_id,omitempty"`
	UserName           *string                  `json:"user_name,omitempty"`
	SparkUiUrl         *string                  `json:"spark_ui_url,omitempty"`
	EndpointId         *string                  `json:"endpoint_id,omitempty"`
	RowsProduced       *int64                   `json:"rows_produced,omitempty"`
	ErrorMessage       *string                  `json:"error_message,omitempty"`
	LookupKey          *string                  `json:"lookup_key,omitempty"`
	Metrics            *queryMetricsWire        `json:"metrics,omitempty"`
	ExecutedAsUserId   *int64                   `json:"executed_as_user_id,omitempty"`
	ExecutedAsUserName *string                  `json:"executed_as_user_name,omitempty"`
	SessionId          *string                  `json:"session_id,omitempty"`
	IsFinal            *bool                    `json:"is_final,omitempty"`
	ChannelUsed        *channelInfoWire         `json:"channel_used,omitempty"`
	PlansState         PlansState               `json:"plans_state,omitempty"`
	StatementType      QueryStatementType       `json:"statement_type,omitempty"`
	WarehouseId        *string                  `json:"warehouse_id,omitempty"`
	Duration           *int64                   `json:"duration,omitempty"`
	ClientApplication  *string                  `json:"client_application,omitempty"`
	QuerySource        *externalQuerySourceWire `json:"query_source,omitempty"`
	CacheQueryId       *string                  `json:"cache_query_id,omitempty"`
	QueryTags          []queryTagWire           `json:"query_tags,omitempty"`
}

func queryInfoToWire(v *QueryInfo) *queryInfoWire {
	if v == nil {
		return nil
	}
	return &queryInfoWire{
		QueryId:            v.QueryId,
		Status:             v.Status,
		QueryText:          v.QueryText,
		QueryStartTimeMs:   v.QueryStartTimeMs,
		ExecutionEndTimeMs: v.ExecutionEndTimeMs,
		QueryEndTimeMs:     v.QueryEndTimeMs,
		UserId:             v.UserId,
		UserName:           v.UserName,
		SparkUiUrl:         v.SparkUiUrl,
		EndpointId:         v.EndpointId,
		RowsProduced:       v.RowsProduced,
		ErrorMessage:       v.ErrorMessage,
		LookupKey:          v.LookupKey,
		Metrics:            queryMetricsToWire(v.Metrics),
		ExecutedAsUserId:   v.ExecutedAsUserId,
		ExecutedAsUserName: v.ExecutedAsUserName,
		SessionId:          v.SessionId,
		IsFinal:            v.IsFinal,
		ChannelUsed:        channelInfoToWire(v.ChannelUsed),
		PlansState:         v.PlansState,
		StatementType:      v.StatementType,
		WarehouseId:        v.WarehouseId,
		Duration:           v.Duration,
		ClientApplication:  v.ClientApplication,
		QuerySource:        externalQuerySourceToWire(v.QuerySource),
		CacheQueryId:       v.CacheQueryId,
		QueryTags:          convertSlice(v.QueryTags, queryTagToWire),
	}
}

func queryInfoFromWire(w *queryInfoWire) *QueryInfo {
	if w == nil {
		return nil
	}
	return &QueryInfo{
		QueryId:            w.QueryId,
		Status:             w.Status,
		QueryText:          w.QueryText,
		QueryStartTimeMs:   w.QueryStartTimeMs,
		ExecutionEndTimeMs: w.ExecutionEndTimeMs,
		QueryEndTimeMs:     w.QueryEndTimeMs,
		UserId:             w.UserId,
		UserName:           w.UserName,
		SparkUiUrl:         w.SparkUiUrl,
		EndpointId:         w.EndpointId,
		RowsProduced:       w.RowsProduced,
		ErrorMessage:       w.ErrorMessage,
		LookupKey:          w.LookupKey,
		Metrics:            queryMetricsFromWire(w.Metrics),
		ExecutedAsUserId:   w.ExecutedAsUserId,
		ExecutedAsUserName: w.ExecutedAsUserName,
		SessionId:          w.SessionId,
		IsFinal:            w.IsFinal,
		ChannelUsed:        channelInfoFromWire(w.ChannelUsed),
		PlansState:         w.PlansState,
		StatementType:      w.StatementType,
		WarehouseId:        w.WarehouseId,
		Duration:           w.Duration,
		ClientApplication:  w.ClientApplication,
		QuerySource:        externalQuerySourceFromWire(w.QuerySource),
		CacheQueryId:       w.CacheQueryId,
		QueryTags:          convertSlice(w.QueryTags, queryTagFromWire),
	}
}

type queryMetricsWire struct {
	TotalTimeMs                       *int64                 `json:"total_time_ms,omitempty"`
	ReadBytes                         *int64                 `json:"read_bytes,omitempty"`
	RowsProducedCount                 *int64                 `json:"rows_produced_count,omitempty"`
	CompilationTimeMs                 *int64                 `json:"compilation_time_ms,omitempty"`
	ExecutionTimeMs                   *int64                 `json:"execution_time_ms,omitempty"`
	ReadRemoteBytes                   *int64                 `json:"read_remote_bytes,omitempty"`
	WriteRemoteBytes                  *int64                 `json:"write_remote_bytes,omitempty"`
	ReadCacheBytes                    *int64                 `json:"read_cache_bytes,omitempty"`
	SpillToDiskBytes                  *int64                 `json:"spill_to_disk_bytes,omitempty"`
	TaskTotalTimeMs                   *int64                 `json:"task_total_time_ms,omitempty"`
	ReadFilesCount                    *int64                 `json:"read_files_count,omitempty"`
	ReadPartitionsCount               *int64                 `json:"read_partitions_count,omitempty"`
	PhotonTotalTimeMs                 *int64                 `json:"photon_total_time_ms,omitempty"`
	RowsReadCount                     *int64                 `json:"rows_read_count,omitempty"`
	ResultFetchTimeMs                 *int64                 `json:"result_fetch_time_ms,omitempty"`
	NetworkSentBytes                  *int64                 `json:"network_sent_bytes,omitempty"`
	ResultFromCache                   *bool                  `json:"result_from_cache,omitempty"`
	PrunedBytes                       *int64                 `json:"pruned_bytes,omitempty"`
	PrunedFilesCount                  *int64                 `json:"pruned_files_count,omitempty"`
	ProvisioningQueueStartTimestamp   *int64                 `json:"provisioning_queue_start_timestamp,omitempty"`
	OverloadingQueueStartTimestamp    *int64                 `json:"overloading_queue_start_timestamp,omitempty"`
	QueryCompilationStartTimestamp    *int64                 `json:"query_compilation_start_timestamp,omitempty"`
	TaskTimeOverTimeRange             *taskTimeOverRangeWire `json:"task_time_over_time_range,omitempty"`
	WorkToBeDone                      *int64                 `json:"work_to_be_done,omitempty"`
	RunnableTasks                     *int64                 `json:"runnable_tasks,omitempty"`
	ProjectedRemainingTaskTotalTimeMs *int64                 `json:"projected_remaining_task_total_time_ms,omitempty"`
	RemainingTaskCount                *int64                 `json:"remaining_task_count,omitempty"`
	ProjectedRemainingWallclockTimeMs *int64                 `json:"projected_remaining_wallclock_time_ms,omitempty"`
	ReadFilesBytes                    *int64                 `json:"read_files_bytes,omitempty"`
}

func queryMetricsToWire(v *QueryMetrics) *queryMetricsWire {
	if v == nil {
		return nil
	}
	return &queryMetricsWire{
		TotalTimeMs:                       v.TotalTimeMs,
		ReadBytes:                         v.ReadBytes,
		RowsProducedCount:                 v.RowsProducedCount,
		CompilationTimeMs:                 v.CompilationTimeMs,
		ExecutionTimeMs:                   v.ExecutionTimeMs,
		ReadRemoteBytes:                   v.ReadRemoteBytes,
		WriteRemoteBytes:                  v.WriteRemoteBytes,
		ReadCacheBytes:                    v.ReadCacheBytes,
		SpillToDiskBytes:                  v.SpillToDiskBytes,
		TaskTotalTimeMs:                   v.TaskTotalTimeMs,
		ReadFilesCount:                    v.ReadFilesCount,
		ReadPartitionsCount:               v.ReadPartitionsCount,
		PhotonTotalTimeMs:                 v.PhotonTotalTimeMs,
		RowsReadCount:                     v.RowsReadCount,
		ResultFetchTimeMs:                 v.ResultFetchTimeMs,
		NetworkSentBytes:                  v.NetworkSentBytes,
		ResultFromCache:                   v.ResultFromCache,
		PrunedBytes:                       v.PrunedBytes,
		PrunedFilesCount:                  v.PrunedFilesCount,
		ProvisioningQueueStartTimestamp:   v.ProvisioningQueueStartTimestamp,
		OverloadingQueueStartTimestamp:    v.OverloadingQueueStartTimestamp,
		QueryCompilationStartTimestamp:    v.QueryCompilationStartTimestamp,
		TaskTimeOverTimeRange:             taskTimeOverRangeToWire(v.TaskTimeOverTimeRange),
		WorkToBeDone:                      v.WorkToBeDone,
		RunnableTasks:                     v.RunnableTasks,
		ProjectedRemainingTaskTotalTimeMs: v.ProjectedRemainingTaskTotalTimeMs,
		RemainingTaskCount:                v.RemainingTaskCount,
		ProjectedRemainingWallclockTimeMs: v.ProjectedRemainingWallclockTimeMs,
		ReadFilesBytes:                    v.ReadFilesBytes,
	}
}

func queryMetricsFromWire(w *queryMetricsWire) *QueryMetrics {
	if w == nil {
		return nil
	}
	return &QueryMetrics{
		TotalTimeMs:                       w.TotalTimeMs,
		ReadBytes:                         w.ReadBytes,
		RowsProducedCount:                 w.RowsProducedCount,
		CompilationTimeMs:                 w.CompilationTimeMs,
		ExecutionTimeMs:                   w.ExecutionTimeMs,
		ReadRemoteBytes:                   w.ReadRemoteBytes,
		WriteRemoteBytes:                  w.WriteRemoteBytes,
		ReadCacheBytes:                    w.ReadCacheBytes,
		SpillToDiskBytes:                  w.SpillToDiskBytes,
		TaskTotalTimeMs:                   w.TaskTotalTimeMs,
		ReadFilesCount:                    w.ReadFilesCount,
		ReadPartitionsCount:               w.ReadPartitionsCount,
		PhotonTotalTimeMs:                 w.PhotonTotalTimeMs,
		RowsReadCount:                     w.RowsReadCount,
		ResultFetchTimeMs:                 w.ResultFetchTimeMs,
		NetworkSentBytes:                  w.NetworkSentBytes,
		ResultFromCache:                   w.ResultFromCache,
		PrunedBytes:                       w.PrunedBytes,
		PrunedFilesCount:                  w.PrunedFilesCount,
		ProvisioningQueueStartTimestamp:   w.ProvisioningQueueStartTimestamp,
		OverloadingQueueStartTimestamp:    w.OverloadingQueueStartTimestamp,
		QueryCompilationStartTimestamp:    w.QueryCompilationStartTimestamp,
		TaskTimeOverTimeRange:             taskTimeOverRangeFromWire(w.TaskTimeOverTimeRange),
		WorkToBeDone:                      w.WorkToBeDone,
		RunnableTasks:                     w.RunnableTasks,
		ProjectedRemainingTaskTotalTimeMs: w.ProjectedRemainingTaskTotalTimeMs,
		RemainingTaskCount:                w.RemainingTaskCount,
		ProjectedRemainingWallclockTimeMs: w.ProjectedRemainingWallclockTimeMs,
		ReadFilesBytes:                    w.ReadFilesBytes,
	}
}

type queryTagWire struct {
	Key   *string `json:"key,omitempty"`
	Value *string `json:"value,omitempty"`
}

func queryTagToWire(v *QueryTag) *queryTagWire {
	if v == nil {
		return nil
	}
	return &queryTagWire{
		Key:   v.Key,
		Value: v.Value,
	}
}

func queryTagFromWire(w *queryTagWire) *QueryTag {
	if w == nil {
		return nil
	}
	return &QueryTag{
		Key:   w.Key,
		Value: w.Value,
	}
}

type taskTimeOverRangeWire struct {
	Entries  []taskTimeOverRangeEntryWire `json:"entries,omitempty"`
	Interval *int64                       `json:"interval,omitempty"`
}

func taskTimeOverRangeToWire(v *TaskTimeOverRange) *taskTimeOverRangeWire {
	if v == nil {
		return nil
	}
	return &taskTimeOverRangeWire{
		Entries:  convertSlice(v.Entries, taskTimeOverRangeEntryToWire),
		Interval: v.Interval,
	}
}

func taskTimeOverRangeFromWire(w *taskTimeOverRangeWire) *TaskTimeOverRange {
	if w == nil {
		return nil
	}
	return &TaskTimeOverRange{
		Entries:  convertSlice(w.Entries, taskTimeOverRangeEntryFromWire),
		Interval: w.Interval,
	}
}

type taskTimeOverRangeEntryWire struct {
	TaskCompletedTimeMs *int64 `json:"task_completed_time_ms,omitempty"`
}

func taskTimeOverRangeEntryToWire(v *TaskTimeOverRangeEntry) *taskTimeOverRangeEntryWire {
	if v == nil {
		return nil
	}
	return &taskTimeOverRangeEntryWire{
		TaskCompletedTimeMs: v.TaskCompletedTimeMs,
	}
}

func taskTimeOverRangeEntryFromWire(w *taskTimeOverRangeEntryWire) *TaskTimeOverRangeEntry {
	if w == nil {
		return nil
	}
	return &TaskTimeOverRangeEntry{
		TaskCompletedTimeMs: w.TaskCompletedTimeMs,
	}
}

type timeRangeWire struct {
	StartTimeMs *int64 `json:"start_time_ms,omitempty"`
	EndTimeMs   *int64 `json:"end_time_ms,omitempty"`
}

func timeRangeToWire(v *TimeRange) *timeRangeWire {
	if v == nil {
		return nil
	}
	return &timeRangeWire{
		StartTimeMs: v.StartTimeMs,
		EndTimeMs:   v.EndTimeMs,
	}
}

func timeRangeFromWire(w *timeRangeWire) *TimeRange {
	if w == nil {
		return nil
	}
	return &TimeRange{
		StartTimeMs: w.StartTimeMs,
		EndTimeMs:   w.EndTimeMs,
	}
}

func convertSlice[T, W any](s []T, conv func(*T) *W) []W {
	if s == nil {
		return nil
	}
	out := make([]W, len(s))
	for i := range s {
		out[i] = *conv(&s[i])
	}
	return out
}
