...
func TestTransport_RoundTrip_WithRequestBody(t *testing.T) {
	seq := fixture.NewSequence(
		fixture.Fixture{
			Method:       "POST",
			Path:         "/api/create",
			ExpectedBody: []byte(`{"name":"new-item"}`),
			Response:     []byte(`{"id":42}`),
		},
	)
	...
	req, _ := http.NewRequest("POST", "http://example.com/api/create", bytes.NewReader([]byte(`{"name":"new-item"}`)))
	resp, err := client.Do(req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestTransport_RoundTrip_OrderedSequence(t *testing.T) {
	seq := fixture.NewSequence(
		fixture.Fixture{Method: "POST", Path: "/api/create", Response: []byte(`{"id":1}`)},
		fixture.Fixture{Method: "GET", Path: "/api/get/1", Response: []byte(`{"id":1,"status":"created"}`)},
...
func TestTransport_Reset(t *testing.T) {
	seq := fixture.NewSequence(
		fixture.Fixture{Method: "GET", Path: "/api/test", Response: []byte(`{"id":1}`)},
	)
