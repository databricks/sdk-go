...
type Fixture struct {
	...
	ExpectedBody any // If set, request body must match when JSON-decoded.

	// Response definition.
	Response   any            // Success response body (will be JSON-encoded).
...
func (s *Sequence) Match(req Request) (Response, error) {
	...
	return buildResponse(req, fixture), nil
}
