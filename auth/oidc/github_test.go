...
func TestGithubIDTokenProvider(t *testing.T) {
	testCases := []struct {
		desc              string
	...
	}{
		{
			...
			wantErrPrefix:     new("missing ActionsIDTokenRequestURL"),
		},
		{
			desc:            "missing request token token",
			tokenRequestUrl: "http://endpoint.com/test?version=1",
			wantErrPrefix:   new("missing ActionsIDTokenRequestToken"),
		},
		{
			...
			wantErrPrefix: new("failed to request ID token from"),
		},
		{
			desc:              "success",
			tokenRequestToken: "token-1337",
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			p := NewGithubIDTokenProvider(cli, tc.tokenRequestUrl, tc.tokenRequestToken)
