...
func TestNewAzureDevOpsIDTokenProvider_success(t *testing.T) {
	for k, v := range defaultEnv {
		t.Setenv(k, v)
	}

	want := &azureDevOpsIDTokenProvider{
	...
	got, gotErr := NewAzureDevOpsIDTokenProvider(nil)
	if gotErr != nil {
		t.Fatalf("NewAzureDevOpsIDTokenProvider() want no error, got error: %v", gotErr)
	}
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("NewAzureDevOpsIDTokenProvider() mismatch (-want +got):\n%s", diff)
	}
}

func TestNewAzureDevOpsIDTokenProvider_missingEnv(t *testing.T) {
	...
	for _, env := range sortedKeys {
		t.Run(fmt.Sprintf("missing %s", env), func(t *testing.T) {
			...
			if !errors.Is(gotErr, wantErr) {
				t.Fatalf("NewAzureDevOpsIDTokenProvider() want error %v, got %v", wantErr, gotErr)
			}
			if got != nil {
				t.Errorf("NewAzureDevOpsIDTokenProvider() want nil, got %v", got)
			}
		})
	}
}

func TestNewAzureDevOpsIDTokenProvider_IDToken(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			TokenProvider, err := NewAzureDevOpsIDTokenProvider(client)
			if err != nil {
				t.Fatalf("NewAzureDevOpsIDTokenProvider() failed: %v", err)
			}

			got, gotErr := TokenProvider.IDToken(context.Background(), "test-audience")
