...
func TestIDTokenProviderFn(t *testing.T) {
	wantToken := &IDToken{Value: "from-func"}
	wantErr := fmt.Errorf("test error")
	wantAud := "func-audience"
	wantCtx := context.Background()

	ts := IDTokenProviderFn(func(gotCtx context.Context, gotAud string) (*IDToken, error) {
		if gotCtx != wantCtx {
...
func TestNewEnvIDTokenProvider(t *testing.T) {
	testCases := []struct {
		desc     string
		envName  string
		envValue string
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			ts := NewEnvIDTokenProvider(tc.envName)
