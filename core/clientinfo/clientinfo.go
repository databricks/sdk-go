...
// to register global metadata before any client is created. Tools that
// assemble their own telemetry can use the Detect functions to obtain
// environment metadata directly.
package clientinfo
...
// DetectUpstream returns the sanitized product and version supplied by a tool
// built on the SDK. Both values are empty unless both upstream environment
// variables are non-empty.
func DetectUpstream() (product, version string) {
	return detectUpstream(os.LookupEnv)
}
...
// DetectRuntimeVersion returns the sanitized Databricks runtime version, or an
// empty string when the process is not running in a detected Databricks runtime.
func DetectRuntimeVersion() string {
	return detectRuntimeVersion(os.LookupEnv)
}
...
// DetectCICDProvider returns the detected CI/CD provider, or an empty string.
func DetectCICDProvider() string {
	return detectCICD(os.LookupEnv)
}

// DetectAgentProvider returns the detected coding-agent provider, or an empty
// string.
func DetectAgentProvider() string {
	return detectAgent(os.LookupEnv)
}

// DetectMetaHarnessProvider returns the detected agent meta-harness provider,
// or an empty string.
func DetectMetaHarnessProvider() string {
	return detectMetaHarness(os.LookupEnv)
}
