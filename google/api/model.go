...
// APIs], [Cloud Endpoints], [gRPC Gateway], and [Envoy] proxy support this
...
// option (google.api.http) = { get: "/v1/{name=messages/*}" }; } } message
...
// message) are classified into three categories: - Fields referred by the path
...
// the parameter name is the field path in the request message. A repeated field
// can be represented as multiple query parameters under the same name. 2. If
...
// [HttpRule.body][google.api.HttpRule.body] is omitted, there is no HTTP
...
// Segments ] "}" ; FieldPath = IDENT { "." IDENT } ; Verb = ":" LITERAL ;
// 
...
// unchanged. Such variables show up in the [Discovery Document] as `{+var}`.
// 
...
// Example:
// 
// http: rules: # Selects a gRPC method and applies HttpRule to it. - selector:
// example.v1.Messaging.GetMessage get: /v1/messages/{message_id}/{sub.subfield}
// 
// ## Special notes
// 
...
// 
// While the single segment variable follows the semantics of [RFC 6570] Section
// 3.2.2 Simple String Expansion, the multi segment variable **does not** follow
// RFC 6570 Section 3.2.3 Reserved Expansion. The reason is that the Reserved
