...
// calls it relies on live in upload_control.go; the part transfers go through
// uc.cloud.
...
import (
	...
	"github.com/databricks/sdk-go/files/v2/internal/cloudstorage"
)
...
func (e *engine) performMultipartUpload(ctx context.Context, uc *uploadContext, token string, r io.Reader, preRead []byte) error {
	currentPart := 1
	...
	for !eof {
		...
		urls, err := e.createPartURLs(ctx, uc.targetPath, token, currentPart, uc.batchSize)
		if err != nil {
			if chunkOffset == 0 {
		...
		for _, purl := range urls {
			...
			case rerr == nil:
				...
				e.c.logger.DebugContext(ctx, "chunk uploaded",
					"part", currentPart, "bytes", chunkLen, "offset", chunkOffset-chunkLen,
	...
	return e.completeMultipart(ctx, uc.targetPath, token, etags)
}
...
func (e *engine) doUploadOnePart(ctx context.Context, uc *uploadContext, part uploadPart, token string, isFirstPart bool) (string, error) {
	start := time.Now()
	urlRetries, slowRetries := 0, 0
	url, headers := part.url, part.headers
	for {
		if url == "" {
			urls, err := e.createPartURLs(ctx, uc.targetPath, token, part.partNumber, 1)
			if err != nil {
		...
		resp, slow, rerr := e.sendPart(ctx, uc, url, headers, part.body, func() time.Duration {
		...
		case rerr == nil:
			...
			e.c.logger.DebugContext(ctx, "part uploaded",
				"part", part.partNumber, "bytes", part.body.Size(), "first", isFirstPart,
		...
		case slow:
			...
			slowRetries++
			url = ""
			e.c.logger.DebugContext(ctx, "part slow, re-issuing on a fresh connection",
				"part", part.partNumber, "attempt", slowRetries+1)
		case cloudstorage.IsURLExpired(rerr):
...
func (e *engine) sendPart(ctx context.Context, uc *uploadContext, url string, headers map[string]string, body cloudstorage.Body, deadline func() time.Duration) (resp *cloudstorage.Response, slow bool, err error) {
	...
	start := time.Now()
	go func() {
		ticker := time.NewTicker(uc.tun.slowCheckInterval)
		defer ticker.Stop()
		for {
			select {
...
func (e *engine) startPartWorkers(pctx context.Context, cancel context.CancelFunc, uc *uploadContext, token string, parallelism int, seed map[int]string, tasks <-chan uploadPart) func() (map[int]string, error) {
	...
	for range parallelism {
		wg.Go(func() {
			for part := range tasks {
				...
				etag, e := e.doUploadOnePart(pctx, uc, part, token, false)
				if e != nil {
					results.setErr(e)
					cancel()
					return
...
func (e *engine) parallelMultipartFromStream(ctx context.Context, uc *uploadContext, token string, r io.Reader) error {
	...
	etag1, err := e.doUploadOnePart(ctx, uc, uploadPart{partNumber: 1, body: cloudstorage.BytesBody(first)}, token, true)
	if err != nil {
	...
	if int64(len(first)) < uc.partSize {
		return e.completeMultipart(ctx, uc.targetPath, token, map[int]string{1: etag1})
	}
	...
	wait := e.startPartWorkers(pctx, cancel, uc, token, uc.parallelism, map[int]string{1: etag1}, tasks)
...
// (see doUploadOnePart).
func (e *engine) parallelMultipartFromReaderAt(ctx context.Context, uc *uploadContext, token string, ra io.ReaderAt) error {
	...
	wait := e.startPartWorkers(pctx, cancel, uc, token, min(uc.parallelism, numParts), map[int]string{}, tasks)
	...
	func() {
		...
		for next := 1; next <= numParts; {
			...
			urls, err := e.createPartURLs(pctx, uc.targetPath, token, next, count)
			if err != nil {
				if pctx.Err() != nil {
	...
	etags, err := wait()
	if producerErr != nil {
		err = producerErr // a producer failure wins over a worker's observed cancellation
	}
	if err != nil {
		if fb, ok := errors.AsType[*fallbackToFilesAPI](err); ok {
