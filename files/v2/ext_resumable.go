...
func (e *engine) performResumableUpload(ctx context.Context, uc *uploadContext, token string, r io.Reader, preRead []byte) error {
	purl, err := e.createResumableURL(ctx, uc.targetPath, token)
	if err != nil {
		return &fallbackToFilesAPI{buffer: preRead, reason: fmt.Sprintf("failed to obtain resumable upload URL: %v", err)}
	}
	if err := e.resumableUploadLoop(ctx, uc, purl, r, preRead); err != nil {
		if aerr := e.abortResumableUpload(ctx, uc, purl); aerr != nil {
			e.c.logger.WarnContext(ctx, "failed to abort resumable upload", "error", aerr)
		}
		return err
	}
	return nil
...
func (e *engine) resumableUploadLoop(ctx context.Context, uc *uploadContext, purl presignedURL, r io.Reader, preRead []byte) error {
	...
	minBuffer := uc.partSize + uc.tun.readAheadBytes
	...
	for {
		...
		case rerr != nil:
			...
			if retryCount >= uc.tun.maxRetries || !cloudstorage.IsRetriable(rerr) {
				return rerr
			}
			retryCount++
			status, qerr := e.resumableStatusQuery(ctx, uc, purl)
			if qerr != nil || status == nil {
				return rerr
			}
			resp = status
		case cloudstorage.IsRetryableStatus(resp.StatusCode):
			if retryCount < uc.tun.maxRetries {
				retryCount++
				if status, qerr := e.resumableStatusQuery(ctx, uc, purl); qerr == nil && status != nil {
					resp = status
				}
			}
		default:
		...
		case resp.StatusCode == http.StatusPermanentRedirect: // 308: chunk accepted, more to come
			...
			if nextOffset == chunkOffset {
				noProgress++
				if noProgress > uc.tun.maxRetries {
					return fmt.Errorf("resumable upload made no progress at offset %d after %d attempts", chunkOffset, noProgress)
				}
			} else {
				// Forward progress recovered from a transient error via the status
				// query: reset both budgets so they bound consecutive stalls, not the
				// total blip count over a long, steadily-advancing upload.
				noProgress = 0
				retryCount = 0
			}
...
func (e *engine) resumableStatusQuery(ctx context.Context, uc *uploadContext, purl presignedURL) (*cloudstorage.Response, error) {
