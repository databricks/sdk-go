...
// credentials, but these tests exercise request construction through a mock
// server that ignores auth headers.
...
func TestCreateTaskWaiter(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name: "success",
			createResponse: &Task{
				TaskId: new(successCreateTaskID),
				Status: &TaskStatus{State: new(TaskStatePending)},
			},
			wantRawResponse: &Task{
				TaskId: new(successCreateTaskID),
				Status: &TaskStatus{State: new(TaskStatePending)},
		...
		},
		{
			...
			createResponse: &Task{Status: &TaskStatus{State: new(TaskStatePending)}},
