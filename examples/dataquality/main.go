...
func main() {
	...
	creds := auth.NewTokenCredentials("example", auth.TokenProviderFn(
	...
	monitor, err := client.CreateMonitor(ctx, &dq.CreateMonitorRequest{
		Monitor: &dq.Monitor{
			...
			ObjectId:   new("main.default.my_table"),
			DataProfilingConfig: &dq.DataProfilingConfig{
				OutputSchemaId: new("main.default"),
				AnalysisConfig: &dq.DataProfilingConfig_AnalysisConfig_Snapshot{
	...
	fmt.Println("\n--- Update Monitor ---")
	updateMask, err := types.NewFieldMask[dq.Monitor]("data_profiling_config.schedule")
	if err != nil {
		log.Fatalf("NewFieldMask: %v", err)
	}
		...
		Monitor: &dq.Monitor{
			...
			DataProfilingConfig: &dq.DataProfilingConfig{
				...
				Schedule: &dq.CronSchedule{
					QuartzCronExpression: new("0 0 12 * * ?"),
					TimezoneId:           new("UTC"),
				},
			},
		},
...
func newMockServer() *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && path == "/api/data-quality/v1/monitors":
			...
			body.DataProfilingConfig.Status = dq.DataProfilingStatus_DataProfilingStatusPending
		...
		case r.Method == http.MethodGet && strings.HasPrefix(path, "/api/data-quality/v1/monitors/") && !strings.Contains(path, "/refreshes"):
			json.NewEncoder(w).Encode(dq.Monitor{
				...
				DataProfilingConfig: &dq.DataProfilingConfig{
					OutputSchemaId: new("main.default"),
					Status:         dq.DataProfilingStatus_DataProfilingStatusActive,
					AnalysisConfig: &dq.DataProfilingConfig_AnalysisConfig_Snapshot{
						Snapshot: dq.SnapshotConfig{},
					},
				},
		...
		case r.Method == http.MethodPatch && strings.HasPrefix(path, "/api/data-quality/v1/monitors/"):
			var body dq.Monitor
			json.NewDecoder(r.Body).Decode(&body)
			body.DataProfilingConfig.Status = dq.DataProfilingStatus_DataProfilingStatusActive
			json.NewEncoder(w).Encode(body)
		...
		case r.Method == http.MethodPost && strings.Contains(path, "/refreshes") && !strings.Contains(path, "/cancel"):
			...
			ref := dq.Refresh{
				...
				ObjectId:   new("main.default.my_table"),
				RefreshId:  new(id),
				State:      dq.RefreshState_MonitorRefreshStatePending,
				Trigger:    dq.RefreshTrigger_MonitorRefreshTriggerManual,
			}
